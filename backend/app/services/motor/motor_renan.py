...
# ✅ Imports relativos do pacote app.services.motor
...
def enviar_relatorio_completo_telegram(resultado):
    ...
    try:
        # ⚠️ xenos_bot não está no novo projeto - função opcional
        ...
        # ⚠️ xenos_bot.enviar_oraculo não está disponível no novo projeto
        # Enviar cada bloco (comentado até integrar bot Telegram)
        ...
        #     enviar_oraculo(mensagem_limpa)  # ⚠️ Não disponível
        #     
        #     # Pequena pausa entre blocos para evitar spam
        #     import time
        #     time.sleep(0.5)
        
        print(f"✅ Relatório gerado em {len(blocos)} blocos organizados! (Telegram bot não configurado)")
        return True
